...
function getCharsetPriority(charset3, accepted, index) {
  ...
  for (var i = 0; i < accepted.length; i++) {
    var spec = specify$3(charset3, accepted[i], index);
...
function getEncodingPriority(encoding3, accepted, index) {
  var priority = { o: -1, q: 0, s: 0 };
  for (var i = 0; i < accepted.length; i++) {
    var spec = specify$2(encoding3, accepted[i], index);
    if (spec && (priority.s - spec.s || priority.q - spec.q || priority.o - spec.o) < 0) {
      priority = spec;
    }
  }
...
function getLanguagePriority(language3, accepted, index) {
  var priority = { o: -1, q: 0, s: 0 };
  for (var i = 0; i < accepted.length; i++) {
    var spec = specify$1(language3, accepted[i], index);
    if (spec && (priority.s - spec.s || priority.q - spec.q || priority.o - spec.o) < 0) {
      priority = spec;
    }
  }
...
var server = polka().use(compression$1({ threshold: 0 }), assetsMiddleware, kitMiddleware, prerenderedMiddleware);
